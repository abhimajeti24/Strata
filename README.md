...
A one-page site for a fictional architecture studio, built as a drawing set: every section is a numbered sheet (DWG 00–07), annotated in mono, dimensioned with 1px hairlines. The brand thesis — *we build silence into cities* — is told through one object: a seven-volume monolith that assembles in the hero, deconstructs into its strata as you scroll the manifesto, and reassembles as a tiny mark in the footer.
...
| Hero | Headline lines rise from clipped containers; monolith fades/scales in; pointer parallax | timeline handoff from preloader, mutable-ref bridge into `useFrame` |
| Manifesto | Pinned 250vh; word-by-word reveal **and** monolith deconstruction driven by one scrubbed progress value | `pin`, `scrub`, shared ref consumed in R3F with 0.08 damping |
...
| Contact | Graphite panel slides up; magnetic CTA arrow; monolith mark reassembles — narrative closes | `gsap.quickTo` spring, scrub |

## The monolith (3D element)

Seven stacked `boxGeometry` volumes, `meshStandardMaterial` in Graphite with a subtle Blueprint fresnel rim injected via `onBeforeCompile`. One key light, one fill, fog matched to Limewash. `dpr` capped at 1.5, all per-frame mutation through refs — zero React state at 60fps. Scroll progress crosses from GSAP to R3F through a plain mutable module (`lib/monolithStore.ts`), never props. The canvas unmounts once scroll passes the manifesto. Devices with ≤4 cores get a pure-CSS monolith instead of WebGL.
...
components/
  canvas/       MonolithScene (visibility + fallback) · MonolithCanvas · Monolith
